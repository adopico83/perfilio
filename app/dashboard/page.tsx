...
export default function DashboardPage() {
  ...
  // Generar respuesta IA para una conversación
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold mb-8">Dashboard - Mensajes Pendientes</h1>
          ...
          {conversations?.map((conv: any) => (
            <div key={conv.id} className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="font-bold text-lg">{conv.customer_name}</h3>
              ...
              {conv.ai_responses?.[0] ? (
                <div className="mb-4">
                  <p className="text-sm font-semibold text-gray-700 mb-2">Respuesta sugerida por IA:</p>
                  <p className="text-gray-800 bg-green-50 p-4 rounded border-l-4 border-green-500">
                    {conv.ai_responses[0].ai_response}
                  </p>
                </div>
              ) : (
                <button
