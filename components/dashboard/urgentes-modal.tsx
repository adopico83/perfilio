    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-white/10 shrink-0">
          <h2 id="urgentes-modal-titulo" className="text-lg font-semibold text-[#ed8936] pr-8">
            Emails urgentes
          </h2>
          <button
            type="button"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4">
          {urgentes.length === 0 ? (
            <p className="text-sm text-white/70">No hay emails urgentes.</p>
          ) : (
              ...
              {urgentes.map((email, idx) => (
                <li key={`${email.fechaIso ?? ''}-${idx}`}>
                  <button
                    ...
                    className="w-full text-left rounded-lg border border-white/10 bg-[#0f2744]/50 hover:bg-[#ed8936]/10 px-3 py-2 transition-colors"
                  >
                    ...
                    <p className="text-[11px] text-[#ed8936] mt-1 tabular-nums">
